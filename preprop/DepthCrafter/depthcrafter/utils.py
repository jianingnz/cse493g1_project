...
def read_video_frames(video_path, process_length, target_fps, max_res, dataset="open"):
    if os.path.isdir(video_path):
        print("==> processing image folder: ", video_path)
        frame_paths = sorted(glob.glob(os.path.join(video_path, "*")))
        images = [PIL.Image.open(p).convert("RGB") for p in frame_paths]
        ...
        print(f"==> resizing to: {(height, width)}")

        resized_images = [
            np.array(img.resize((width, height), PIL.Image.BICUBIC)).astype("float32") / 255.0
            for img in images
        ]
        if process_length != -1:
            resized_images = resized_images[:process_length]
        frames = np.stack(resized_images)
        print(f"==> final processing shape: {frames.shape}")
        fps = target_fps if target_fps != -1 else 15  # default fps if not specified
        return frames, fps
    else:
        if dataset == "open":
